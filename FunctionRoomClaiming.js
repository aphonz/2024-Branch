...
manageRoomClaiming: function manageRoomClaiming() {
    ...
    const ownedRooms = _.filter(Game.rooms, room => room.controller && room.controller.my && room.level > 4);
...
spawnClaimingUnits: function spawnClaimingUnits(spawn) {
   ...
   // Prevent premature failure before any claimer has been spawned
    ...
    if ( claimData.successfulClaim) {
        ...
        else FunctionRoomClaiming.placeSpawn(targetRoom);
    }
  },
 ...
 placeSpawn: function placeSpawn(roomName) {
    ...
    if (!spawnExists) {
        const spawnPos = new RoomPosition(flag.pos.x, flag.pos.y + 2, roomName);
        ...
        if (constructionSites.length === 0) {
            ...
            console.log(`Spawn construction site already exists at ${spawnPos}`);
        }
    } else {
